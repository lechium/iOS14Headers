//
//     Generated by classdumpios 1.0.1 (64 bit) (iOS port by DreamDevLost)(Debug version compiled Sep 26 2020 13:48:20).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <HomeUI/HUPCSetupAccessoryContentController.h>

#import "HSSetupStep-Protocol.h"
#import "HUPreloadableViewController-Protocol.h"

@class HFDiscoveredAccessory, HULinkedApplicationLibraryItem, NAFuture, NSString;
@protocol HSSetupStepDelegate;

@interface HSPCSetupConfirmationThirdPartyViewController : HUPCSetupAccessoryContentController <HSSetupStep, HUPreloadableViewController>
{
    id <HSSetupStepDelegate> _delegate;	// 8 = 0x8
    HFDiscoveredAccessory *_discoveredAccessory;	// 16 = 0x10
    NSString *_bundleID;	// 24 = 0x18
    HULinkedApplicationLibraryItem *_linkedApplicationItem;	// 32 = 0x20
}

+ (id)_descriptionForAppName:(id)arg1 appPublisher:(id)arg2 accessoryName:(id)arg3;	// IMP=0x000000010005e130
- (void).cxx_destruct;	// IMP=0x000000010005e3cc
@property(readonly, nonatomic) HULinkedApplicationLibraryItem *linkedApplicationItem; // @synthesize linkedApplicationItem=_linkedApplicationItem;
@property(readonly, nonatomic) NSString *bundleID; // @synthesize bundleID=_bundleID;
@property(readonly, nonatomic) HFDiscoveredAccessory *discoveredAccessory; // @synthesize discoveredAccessory=_discoveredAccessory;
@property(nonatomic) __weak id <HSSetupStepDelegate> delegate; // @synthesize delegate=_delegate;
- (id)hu_preloadContent;	// IMP=0x000000010005de78
@property(readonly, nonatomic) NAFuture *readyToDisplayFuture;
@property(readonly, nonatomic) long long setupState;
- (id)initWithTitle:(id)arg1 detailText:(id)arg2 icon:(id)arg3 item:(id)arg4;	// IMP=0x000000010005dda8
- (id)initWithDiscoveredAccessory:(id)arg1 bundleID:(id)arg2;	// IMP=0x000000010005d950

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

